...
public sealed class ReflagClient : IAsyncDisposable
{
    ...
    private readonly IReflagHttpTransport _transport;
    private readonly HttpClientTransport? _ownedTransport;
    private readonly HttpClientTransport? _ownedPushTransport;
    ...
    public ReflagClient(ReflagClientOptions? options = null)
    {
        ...
        if (options.HttpTransport is not null)
        {
            _transport = options.HttpTransport;
            if (syncMode == ReflagFlagsSyncMode.Push)
            {
                _ownedPushTransport = new HttpClientTransport(options.HttpClient, ownsHttpClient: options.HttpClient is null);
            }
        }
        else
        {
            _ownedTransport = new HttpClientTransport(options.HttpClient, ownsHttpClient: options.HttpClient is null);
        ...
        _flagsSyncController = syncMode switch
        {
            ...
            ReflagFlagsSyncMode.Push => new PushFlagsSyncController(
                ...
                _config.Headers,
                _ownedTransport ?? _ownedPushTransport ?? throw new InvalidOperationException("push sync mode requires an HTTP client transport for SSE.")),
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        if (!ReferenceEquals(_ownedPushTransport, _ownedTransport))
        {
            _ownedPushTransport?.Dispose();
        }
